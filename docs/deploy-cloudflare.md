...
## 8. 新規登録（メールアドレス確認）とパスワード再設定

ログイン画面の「新規登録」から、**メールアドレスとパスワードだけで登録**できます。管理者の
承認は不要ですが、**メールアドレスの確認（本人のメールに届くリンクのクリック）が必要**です。

1. `/register` でメールアドレス（＝ログインID）とパスワードを入力 → `/api/auth/signup` が
   **仮登録**（`status=pending`）としてアカウントを作成し、確認メールを送信
2. 届いたメールのリンク（`/api/auth/verify?token=...`）を開くと **有効化**（`status=active`）
   され、14日間の無料トライアルが始まります
3. 確認が済むまではログインできません（未確認でログインすると「メールアドレスの確認が
   完了していません」と表示され、その場から確認メールを再送できます）。確認リンクの有効期限は 24 時間

パスワードを忘れた場合は、ログイン画面の「パスワードをお忘れの方」からメールアドレスを
入力すると、再設定用リンクがメールで届きます（`/api/auth/forgot-password` →
リンク先 `/reset` で新しいパスワードを設定）。再設定リンクの有効期限は 1 時間です。

### 確認メール・再設定メールを有効にする（Resend）

確認メールと再設定リンクは不特定の利用者のメールアドレス宛に送るため、外部配信サービス
**Resend** を使います（Cloudflare Email Routing は検証済みの自分のアドレス宛にしか送れないため
使いません）。
...
> ⚠️ **`RESEND_API_KEY` は新規登録に必須です。** 未設定だと確認メールが送れず、新規ユーザーは
> アカウントを有効化できません（仮登録のまま・ログイン不可）。セルフ登録を使う場合は必ず
> 設定してください。パスワード再設定メールも同様に Resend 経由で送信します。
> なお、存在しないメールアドレスかどうかを漏らさないため、確認・再設定メールの送信画面では
> 常に「送信しました」と表示されます。

> ⚠️ **Cloudflare Access（手順5/A-5）を使っている場合の注意**
> Access でドメイン全体を保護していると、`/register`・`/forgot`・`/reset` や `/api/auth/*`
> （確認・再設定リンクを含む）に**部外者が到達できず、新規登録やパスワード再設定が
> 機能しません**。**セルフ登録を使うなら この Worker では Cloudflare Access を外す**
> （アプリ内の ID＋パスワードで保護）ことを推奨します。Access を残す場合は、これらのパスに
> バイパス（Service Auth / Public）を設定してください。
